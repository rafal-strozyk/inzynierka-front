<template>
  <section class="max-w-7xl mx-auto">
    <div class="flex max-sm:flex-col justify-between items-center gap-4 mb-4">
      <h1 class="text-3xl font-bold mb-0">Zgłoszenia</h1>
      <generic-button
        v-if="isMyRole(['tenant'])"
        :callback="() => router.push({ name: 'AddTicket' })"
        :iconPath="TicketIconSVG"
...
import TicketIconSVG from "@/assets/img/icons/tickets_white.svg";
...
async function fetchUsers() {
  ...
  const [response, error] = await catchAxiosError<{
    ...
    window.API.get(
      `/owner/tenants?page=${queryParams.value.page}&per_page=${queryParams.value.per_page}`,
    ),
  );
