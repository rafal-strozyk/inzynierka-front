...
const filteredColumns = computed(() => columns.value.filter((column) => column.label !== "id"));

type PropertyData = {
  [key in (typeof columns.value)[number]["label"]]: string | number | boolean;
};

const data = ref<PropertyData[]>([
  {
    id: "liczba/uuid",
    name: "string",
    address: "string w formacie: [street] [street_number], m. [apartment_number]",
    city: "string",
    rent_cost: "Zwyka liczba, np. 123.09",
    utilities_cost: "Zwyka liczba, np. 123.09",
    has_balcony: "0/1 albo true/false",
    has_terrace: "0/1 albo true/false",
  },
]);
</script>
